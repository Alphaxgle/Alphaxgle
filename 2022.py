...
def masud(u):
        for e in u + "\n":sys.stdout.write(e);sys.stdout.flush();time.sleep(0)
def clear():
	os.system('clear')
def back():
	login()
...
def menu(my_name,my_id):
	...
	print('        \n    [\033[1;97m\033[1;41m  @xglle   \033[0m\033[1;93m]\n')
	print('\n    [\033[1;97m\033[1;41m  ○●○●○●○●   \033[0m\033[1;93m]\n')
	print('%s[%s1%s]%s PUBLIC CRACKER-تايديا %s[%sON%s]'%(P,H,P,H,P,H,P))	
	print('%s[%s2%s]%s FILE CRACKER-فلم %s[%sON%s]'%(P,H,P,H,P,H,P))	
#	print('%s[%s3%s]%s CREATE | FILE  %s[%sON%s]'%(P,H,P,H,P,H,P))	
	print('%s[%sB%s]%s EXIT-جورخ %s[%sOut%s]'%(P,H,P,H,P,H,P))
	MRBX = input('%s[%s?%s]%s select menu-رتخا %s : '%(N,H,N,H,M))
	if MRBX in ['1','01']:
		public()
	elif MRBX in ['2','02']:
		File2()
